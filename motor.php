...
if($ejecuta == "movimiento"){
	...
	if ($tarea_actual[0] == 0){
		$query_exitencias = mysql_query('SELECT id, '.$tarea_actual["unidad"].' FROM existencias_'.$tarea_actual["tipo"].' WHERE dueño=\''.$tarea_actual["jugador"].'\' AND planetaactual=\''.$tarea_actual["planeta"].'\'') or die(mysql_error());
		$existencias = mysql_fetch_array($query_exitencias);
		if (empty($existencias)){
			mysql_query('INSERT INTO existencias_'.$tarea_actual["tipo"].' (planetaactual, dueño, '.$tarea_actual["unidad"].') VALUES (\''.$tarea_actual["planeta"].'\', \''.$tarea_actual["jugador"].'\', \''.$tarea_actual["cantidad"].'\')') or die(mysql_error());
		}else{
			if ($existencias[1] != NULL){
				$cantidad_total = $existencias[1] + $tarea_actual["cantidad"];
				mysql_query('UPDATE existencias_'.$tarea_actual["tipo"].' SET '.$tarea_actual["unidad"].'=\''.$cantidad_total.'\' WHERE dueño=\''.$tarea_actual["jugador"].'\' AND planetaactual=\''.$tarea_actual["planeta"].'\'') or die(mysql_error());
			}else{
				mysql_query('UPDATE existencias_'.$tarea_actual["tipo"].' SET '.$tarea_actual["unidad"].'=\''.$tarea_actual["cantidad"].'\' WHERE dueño=\''.$tarea_actual["jugador"].'\' AND planetaactual=\''.$tarea_actual["planeta"].'\'') or die(mysql_error());
			}
		}
		mysql_query('DELETE FROM cons_pend WHERE id = \''.$cons_pend["id"].'\'') or die(mysql_error());
	}else{
